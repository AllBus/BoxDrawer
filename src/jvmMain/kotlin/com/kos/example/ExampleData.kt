package com.kos.example

import androidx.annotation.StringRes
import com.google.gson.GsonBuilder
import com.google.gson.annotations.SerializedName
import com.kos.example.ExampleData.ApiError.ApiException
...
//    }
        ...
        phones.forEach { text ->
            ...
            val p = text
            val s = p.sumOf{ char -> char.code }
            val t = text.fold(757) { acc, char -> (acc shl 2) + char.code * 47 }
            val index = abs(t) % 25
            println("$p $s ${s%25} $t ${abs(t)% 25} ")
//        val h = MessageDigest.getInstance("SHA-256")
//        h.update(p.toByteArray())
//        val rr = h.digest()
//        val r = bytesToHex(rr)
//        var s: Int = 0
//        var sp = 0
//        rr.forEach { n: Byte ->
//            println("$s ($sp) $n (${n.toUByte().toInt()}) -> ${s+n} (${sp+n.toUByte().toInt()})")
//            s+=n
//            sp=sp+n.toUByte().toInt()
//        }
//        val s1 = rr.sum()
//        val s2 = rr.sumOf{ it.toUByte().toInt() }
//        println("$s1 $s2")
//        println("$p ${r} ${rr.last()} ${rr.last() % 25}")
        }
...
//        currentRangeMask = mask
    ...
    class CorTest(){
        ...
        suspend fun one(count:Int, i:Int) = withContext(scope.coroutineContext){
            println("message_from $count - $i")
        }
    ...
    fun main7(args: Array<String>) {
        CorTest()
//    val t = "gpbapp://transferSettings/sbp/defaultCardАndBank\""
//
//    println(t.map { it.code }.joinToString())
//    println()
//
...
//    println(d)
    ...
    fun nonvec222() {
        vectors.maina()
        val rr = Retro(screen = ScreenTypeResponse.SCREEN)
        ...
        repeat(count) {
            result.add(
                Retro(
                    ia = it,
                    screen = ScreenTypeResponse.POPUP
                )
            )
        }
...
//            )
    ...
    sealed class ApiError(
        ...
        class ApiException(
            ...
            cause: Throwable? = null,
            val url: String? = null,
        ) : ApiError(0, message, cause)
    }

    fun main3(args: Array<String>){
        val b = AtomicBoolean(true)
        val c = AtomicBoolean(true)
        println(b.compareAndSet(false, false))
        println(b.get())
        println("---")
        println(c.compareAndSet(true, false))
        println(c.get())
        val exc = ApiException(1, "Abc clause", NullPointerException(), "omni-oks")
        val aaa =exc.toString()
        val dddd =exc.message
        println(aaa)
        println(dddd)
    }
...
sealed class Result<out T> : State<T>() {
