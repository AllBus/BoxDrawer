...
data class BezierCurve(val p0: Vec2, val p1: Vec2, val p2: Vec2, val p3: Vec2): PathElement {
    ...
    override fun translate(xy: Vec2): BezierCurve {
        return BezierCurve(
            p0 + xy,
            p1 + xy,
            p2 + xy,
     ...
     */
    fun subBezier(startOffset: Double, endOffset: Double): BezierCurve {
        ...
        return BezierCurve(newP0, newP1, newP2, newP3)
    }
     ...
     */
    fun parallelCurve(offsetDistance: Double, numSamples: Int = 100): BezierCurve {
        if (isStraightLine()) {
            ...
            return fitBezierCurveToPoints(newPoints)
        }
    }
    ...
    private fun fitBezierCurveToPoints(points: List<Vec2>): BezierCurve {
        if (points.size < 4) {
            throw IllegalArgumentException("At least 4points are required to fit a Bezier curve.")
        }
        ...
        val A = Array(points.size) { DoubleArray(4) }
        val b = Array(points.size) { DoubleArray(2) }
    ...
    private fun solveLeastSquares(A: Array<DoubleArray>, b: Array<DoubleArray>): Array<DoubleArray> {
